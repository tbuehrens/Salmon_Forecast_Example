---
title: Columbia R. Sp. Ck. Forecast 
author: 
output:
  html_document:
    code_folding: hide
...
# dat<-readxl::read_xlsx(here("Data","CKL_2019.xlsx")) %>% 
 dat<-read_csv(here("Data","UpriverSpring_Return.csv")) %>%  
         rename(`Jack Chinook`=Age3,
                Year=ReturnYear,
                runsize_obs=Return )
# dat
...
dat2<-dat%>%
  ...
  mutate(
    ...
    lag1_JCK = lag(c(scale(log(`Jack Chinook`)))),
...
TY <- dim(dat)[1]-14 #training years (years of training data)
...
results<-inseason_forecast_v2(dat,
                  leave_yrs=15,
