---
title: Col. R. Sockeye Forecast @ BON (5/1 - 12/31)
...
dat<-read_csv("http://www.cbr.washington.edu/dart/cs/php/rpt/adult_annual.php?sc=1&outputFormat=csv&proj=BON&startdate=5%2F1&enddate=12%2F31")%>%
  arrange(Year)%>%
  dplyr::select(Year, Sockeye)%>%
  dplyr::rename(runsize_obs=Sockeye)%>%
...
dat<-dat%>%
  ...
  mutate(
    ...
    lag1_log_SAR1 = lag1_log_SAR1,
    lag1_log_SAR2 = lag1_log_SAR2,
    lag1_NPGO = lag1_NPGO,
    lag2_NPGO = lag2_NPGO,
    lag1_PDO = lag1_PDO,
    lag2_PDO = lag2_PDO,
...
vars<-c("pink_ind","lag1_NPGO") #"lag1_log_SAR1" #"lag1_PDO" "lag2_NPGO" "lag1_SAR1" #what regression variables will we use?
dat<-data.frame(dat)%>%
...
ensemble_results<-evaluate_forecasts_with_ensembles(forecasts = forecasts, observations = dat,stack_metric=stack_metric)
#=================================
