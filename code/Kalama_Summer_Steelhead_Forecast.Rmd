---
title: Kalama R. STH Escapement
author: Jeremy Wilson (jeremy.wilson@dfw.wa.gov) and Thomas Buehrens (tbuehrens@dfw.wa.gov)
output:
  html_document:
    code_folding: hide
    fig_caption: yes
...
#==========================
dat<-read_excel("data/Kal_Esc.xlsx")%>%
  arrange(Year)%>%
...
#   filter(Pop=="SNA")
# 
# POA1<-data.frame(SAR1=gam(cbind(ocean1Count,ocean2Count)~s(OutmigrationYear,k=(dim(PIT)[1])),family=binomial,data=PIT)$fitted)
# 
# PIT<-PIT%>%
#   bind_cols(POA1)
#===============================
...
vars<-c("lag2_NPGO", "log_BONlatewinters","diff_log_BONlatewinters") #what regression variables will we use?
dat<-data.frame(dat)%>%
