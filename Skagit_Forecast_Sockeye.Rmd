...
NPGO<-read_table("http://www.o3d.org/npgo/npgo.php",skip=29,col_names=F,comment="#")%>%
  ...
  mutate(Year=Year+2, lag2_NPGO = NPGO,lag3_NPGO = lag(NPGO,1))%>%
  dplyr::select(Year=Year,lag2_NPGO,lag3_NPGO)

write.csv(NPGO, "NPGO.csv")
...
dat <- dat%>%
  left_join(SAR)%>%
  left_join(Smolts)%>%
  dplyr::select(Year=Year,runsize_obs,lag1_log_SAR2,lag2_log_smolts)
  
  #left_join(NPGO)%>%
  #dplyr::select(Year=Year,runsize_obs,lag1_log_SAR2,lag3_NPGO,lag2_log_smolts)
...
#vars<-c("lag1_log_SAR2","lag2_log_smolts","lag3_NPGO") #what regression variables will we use?

vars<-c("lag1_log_SAR2","lag2_log_smolts") #what regression variables will we use?
dat<-data.frame(dat)%>%
...
ARIMA100_withcov_smolts_SAR1<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag1_log_SAR2","lag2_log_smolts")%>%as.matrix(),
            # xreg=dat%>%dplyr::select("lag1_log_SAR2","lag2_log_smolts","lag3_NPGO")%>%as.matrix(),
            h=FY,
            min=TY,
            type="ARIMA",
            order=c(1,0,0)
...
ARIMA111_withcov_smolts_SAR1<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag1_log_SAR2","lag2_log_smolts")%>%as.matrix(),
            # xreg=dat%>%dplyr::select("lag1_log_SAR2","lag2_log_smolts","lag3_NPGO")%>%as.matrix(),
            h=FY,
...
ARIMA010_withcov_smolts_SAR1<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag1_log_SAR2","lag2_log_smolts")%>%as.matrix(),
            # xreg=dat%>%dplyr::select("lag1_log_SAR2","lag2_log_smolts","lag3_NPGO")%>%as.matrix(),
            h=FY,
...
#             order=c(1,1,1)
# )
ARIMA101_withcov_smolts_SAR1<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag1_log_SAR2","lag2_log_smolts")%>%as.matrix(),
            # xreg=dat%>%dplyr::select("lag1_log_SAR2","lag2_log_smolts","lag3_NPGO")%>%as.matrix(),
            h=FY,
...
ARIMA001_withcov_smolts_SAR1<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag1_log_SAR2","lag2_log_smolts")%>%as.matrix(),
            # xreg=dat%>%dplyr::select("lag1_log_SAR2","lag2_log_smolts","lag3_NPGO")%>%as.matrix(),
            h=FY,
...
ensemble_results$final_model_weights%>%
  dplyr::select(Model, RMSE_weight, MAPE_weight, MSA_weight)%>%
...
best_sd<-best_model%>%filter(Year==max(best_model$Year))%>%dplyr::select(sd)%>%unlist()
