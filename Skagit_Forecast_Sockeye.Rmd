...
dat <- dat%>%
  ...
  left_join(Smolts)%>%
  left_join(NPGO)%>%
  dplyr::select(Year=Year,runsize_obs,lag1_log_SAR1,lag2_NPGO,lag2_log_smolts)
...
vars<-c("lag1_log_SAR1","lag2_log_smolts","lag2_NPGO") #what regression variables will we use?
dat<-data.frame(dat)%>%
...
ARIMA100_withcov_NPGO_smolts_SAR1<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag1_log_SAR1","lag2_log_smolts","lag2_NPGO")%>%as.matrix(),
            h=FY,
            min=TY,
            type="ARIMA",
            order=c(1,0,0)
...
ARIMA100_withcov_SAR1<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag1_log_SAR1")%>%as.matrix(),
            h=FY,
            min=TY,
            type="ARIMA",
            order=c(1,0,0)
