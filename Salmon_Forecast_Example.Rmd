...
k = 5 #this is the model-averaging weighting exponent--default is 1, larger numbers will tend to more heavily weight "best" models over lower ranked models
```
...
dat3<-dat2%>%
  ...
  summarise(RMSE = sqrt(mean(error^2)),
            ...
            MSA = 100*exp(median(abs(log(Chinook.y/Chinook.x)))-1)
              )%>%
  arrange(MSA)%>%
...
ensembles<-dat2%>%
  ...
  pivot_longer(names_to = "Model",
               cols=c("MSA_weighted","RMSE_weighted","MAPE_weighted"),
               values_to = "Chinook")
...
ggplot(data=ensembles,aes(x=Year,y=Chinook,color=Model))+
  ...
  geom_line(size=1)+
  scale_color_brewer(palette="Spectral")+
  geom_point(data=dat%>%mutate(Model="Observed"),mapping=aes(x=Year,y=Chinook),size=2)+
  theme_bw()
```
