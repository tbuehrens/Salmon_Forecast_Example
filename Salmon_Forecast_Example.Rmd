...
ggplot(data=dat2,aes(x=Year,y=Chinook,color=Model))+
  labs(title="One-Year-Ahead Forecast Performance")+
  geom_line(size=1)+
  scale_color_brewer(palette="Spectral")+
...
dat3<-dat2%>%
  ...
  arrange(MSA)%>%
  #could really benefit from adding a function to calculate 'stacking weights'
...
# Stacking Weights
```{r stack_weights, message = FALSE, warning = FALSE,results = "show"}
...
#print results
dat3%>%
  kbl(caption = "Table 2. One Year Ahead Forecast Performance & model weights",digits =3)%>%
  kable_classic(full_width = F, html_font = "Cambria")
```

# Ensemble Forecasts
Here we use model weights based on forecast performance to calculate model-averaged "ensemble" forecasts and look at their performance.
```{r ensemble_forecasts, message = FALSE, warning = FALSE,results = "show"}
#==================================================================================
# develop ensemble models by using weights based on performance of candidate models
#==================================================================================
ensembles<-dat2%>%
  left_join(dat3)%>%
...
ensembles%>%
  ...
  kbl(caption = "Table 3. Ensemble forecasts for final year of data.",digits =0)%>%
  kable_classic(full_width = F, html_font = "Cambria")
...
dat4<-ensembles%>%
  ...
  arrange(MSA)#%>%
  # mutate(
  #   MSA_weight=(1/MSA)^k/sum((1/MSA)^k), 
  #   RMSE_weight =(1/RMSE)^k/sum((1/RMSE)^k),
  #   MAPE_weight =(1/MAPE)^k/sum((1/MAPE)^k)
  # )
dat4%>%
  kbl(caption = "Table 4. One-year ahead performance of ensemble forecasts (using final year weights; not exact year-specific weights that would have been used if annually weighted).",digits =3)%>%
  kable_classic(full_width = F, html_font = "Cambria")
...
```
# Performance Note
The performance of the ensembles should be better than the individual members. The stacking weighted ensemble in particular, is guaranteed to be at least as good as the best individual member. However, looking at their performance of the entire left out year-set, their performance is imperfectly estimated because it uses the final weights, whereas the actual weights used to make one-ahead foreacsts in each year would have been different than the final weights.