---
title: Col. R. B-Run S. Sth. Forecast @ Mouth
...
#====================================
dat<-read_excel("data/CR_Steelhead_returns_forecasts.xlsx")%>%
  dplyr::filter(`Management Stock` == "B-Index",Escapement == "Total")%>%
  pivot_wider(names_from = Count_type, values_from = Count, id_cols = everything())%>%
  arrange(Year)%>%
  dplyr::select(Year, Actual)%>%
  dplyr::rename(runsize_obs=Actual)%>%
...
SAR2<-data.frame(SAR2= c(gam(cbind(ocean2Count,juvCount-ocean2Count)~s(OutmigrationYear,k=(dim(PIT)[1]-1)),family=binomial,data=PIT)$fitted,NA))
PIT<-PIT%>%
  bind_cols(SAR1,SAR2)%>%
  dplyr::select(!c("Pop"))
...
#=========================================================
OceanSalmon<-read_csv("data/pink_covariates_1952-2020.csv")%>%
...
dat<-dat%>%
  ...
  mutate(
    ...
    lag2_SAR2 = lag(scale(log(SAR1)),2),
    lag1_NPGO = lag(NPGO),
    lag2_NPGO = lag(NPGO,2),
    lag1_TotalPinks = lag(scale(tot_pk_num)),
    lag2_TotalPinks = lag(scale(tot_pk_num),2),
    lag0_TotalPinks = scale(tot_pk_num)
    )

#select variables to include in analysis and subset data
vars<-c("lag0_TotalPinks","lag1_NPGO") #what regression variables will we use? "lag1_SAR1","lag2_SAR2", "lag1_NPGO","lag2_NPGO","lag1_TotalPinks",
dat<-data.frame(dat)%>%
