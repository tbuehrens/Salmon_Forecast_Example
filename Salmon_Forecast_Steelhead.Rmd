...
tsCV2<-function(y,Year,xreg,fc,min,h,order){
  ...
  for(i in min:length(y)){
    forecast[i+1,] = fc(y=y[1:i],Year=Year[1:i],xreg=xreg,h=h,order=order)
  }
  results<-forecast%>%
    remove_rownames()
  return(results)
...
ggplot(data=dat2,aes(x=Year,y=Steelhead,color=Model))+
  ...
  geom_line(size=1)+
  scale_color_brewer(palette="Spectral")+
  geom_point(data=dat%>%mutate(Model="Observed"),mapping=aes(x=Year,y=Steelhead),size=2,color="black")+
  theme_bw()
```
...
for(i in 2: length(yrrange[1]:yrrange[2])){
  ...
  tdat2<-dat2%>%
    ...
    left_join(tdat)%>%
    mutate(MSA_weighted = Steelhead * MSA_weight,
           RMSE_weighted = Steelhead * RMSE_weight,
           MAPE_weighted = Steelhead * MAPE_weight,
           Stack_weighted = Steelhead * Stacking_weight,
           )%>%
    group_by(Year)%>%
...
#==============
ggplot(data=ensembles,aes(x=Year,y=Steelhead,color=Model))+
