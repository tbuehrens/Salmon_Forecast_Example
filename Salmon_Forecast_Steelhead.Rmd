...
tsCV2<-function(y,Year,xreg,fc,min,h,order){
  ...
  for(i in min:length(y)){
    forecast[i+1,] = fc(y=y[1:i],Year=Year[i+1],xreg=xreg,h=h,order=order)
  }
  results<-forecast%>%
    remove_rownames()
  return(results)
...
dat<-read_csv("http://www.cbr.washington.edu/dart/cs/php/rpt/adult_annual.php?sc=1&outputFormat=csv&proj=BON&startdate=5%2F15&enddate=12%2F31")%>%
  ...
  mutate(`Steelhead`=ifelse(as.numeric(Year)<=as.numeric(format(Sys.Date(),"%Y")),`Steelhead`,NA))%>%
  filter(!is.na(Year))
...
dat<-dat%>%
  ...
  mutate(
    ...
    )%>%filter(!is.na(lag2_NPGO) & !is.na(lag1_SAR1)
    )
#look at our data  
...
dat<-data.frame(dat)%>%dplyr::select(Year,Steelhead,all_of(vars))
#for gam fits and r-arima fits
xreg=as.matrix(dat%>%dplyr::select(all_of(vars)))
...
forecasts<-list(
  ARIMA100 = ARIMA100
  ,ARIMA100_withcov = ARIMA100_withcov
  ,ARIMA111_withcov = ARIMA111_withcov
  ,ARIMA111 = ARIMA111
  ,ARIMA010_withcov = ARIMA010_withcov
  ,ARIMA010 = ARIMA010
  ,ARIMA101_withcov = ARIMA101_withcov
  ,ARIMA101 = ARIMA101
  ,gam = gam
  #,gp_horseshoe = LFO11
)%>%
...
dat2<-forecasts%>%
  mutate(Steelhead=exp(Estimate))%>%
  dplyr::select(Year,Model,Steelhead)%>%
...
forecasts%>%
  ...
  dplyr::select(!SE)%>%
  mutate(Estimate=exp(Estimate),U95=exp(U95),L95=exp(L95))%>%
...
#===========================
ggplot(data=dat2,aes(x=Year,y=Steelhead,color=Model))+
...
dat3<-dat2%>%
  ...
  dplyr::select(Year,Model,Steelhead.x,Steelhead.y)%>%
  mutate(error=Steelhead.y-Steelhead.x)%>%
  filter(!is.na(error))%>%
  group_by(Model)%>%
  summarise(RMSE = sqrt(mean(error^2)),
            MAPE = mean(abs(error/Steelhead.y))*100,
            MSA = 100*(exp(mean(abs(log(Steelhead.y/Steelhead.x))))-1)
              )%>%
  arrange(MSA)
...
for(i in 2: length(yrrange[1]:yrrange[2])){
  ...
  stackdat<-dat2%>%
    filter(Year %in% years)%>%
    filter(!is.na(Steelhead))%>%
    pivot_wider(names_from = Model, values_from = Steelhead,id_cols = Year)%>%
...
#=============================
dat4<-bind_rows(forecasts%>%mutate(Estimate=exp(Estimate)),ensembles)%>%
