#fit model function for ARIMA_auto models
fc4 <- function(y,Years, h, xreg){
  if(!is.null(xreg)){
    if(ncol(xreg)>1){
      X <- xreg[1:length(y),]
    ...
    fit<-auto.arima(y,xreg=X,method="ML",seasonal=FALSE,stationary=FALSE)
    # fit<-auto.arima(y,xreg=X,method="ML",seasonal=FALSE,stationary=FALSE,
    #                 stepwise=FALSE,parallel=TRUE,num.cores=4,
    ...
    fit<-auto.arima(y,method="ML",seasonal=FALSE,stationary=FALSE)
    # fit<-auto.arima(y,method="ML",seasonal=FALSE,stationary=FALSE,
    #                 stepwise=FALSE,parallel=TRUE,num.cores=4,
    #                 max.p=3,max.q=3,max.d=1,
    #                 allowdrift=FALSE,allowmean=TRUE)
