...
fc2 <- function(y, Year, h, xreg, order){
  if(!is.null(xreg)){
    ...
    formula = as.formula(paste0("exp(y) ~ s(year, m=2, bs='ps',","k =",length(y),") +",paste(colnames(xreg),collapse = "+")))
...
dat<-data.frame(dat)%>%
  #filter(Year>1999)%>% #get rid of years with sketchy
...
forecasts<-list(
  ...
  ,ARIMA001_withcov = ARIMA001_withcov$results
  ,ARIMA001 = ARIMA001$results
  ,gam = gam$results
