...
#====================================
dat<-read_csv("https://raw.githubusercontent.com/tbuehrens/Skagit-River-Steelhead-Forecast/master/data/skagit_sthd_esc.csv")%>%
  left_join(read_csv("https://raw.githubusercontent.com/tbuehrens/Skagit-River-Steelhead-Forecast/master/data/skagit_sthd_catch.csv"))%>%
  mutate(Steelhead=catch+escapement)%>%
...
#=========================================================
# PIT<-read_csv("Columbia_Steelhead_SAR_DART_11_1_2021.csv")%>%
...
# indicators<-read_csv("Stoplight csv.csv",skip=1)%>%
...
```{r set_parameters, message = FALSE, warning = FALSE,results = "show"}
#select variables to include in analysis and subset data
vars<-c("lag2_NPGO","lag2_CFS") #what regression variables will we use?
dat<-data.frame(dat)%>%
  #ilter(Year>1999)%>% #get rid of years with sketchy
  dplyr::select(Year,Steelhead,all_of(vars))%>%
  filter(
    across(
      .cols = all_of(vars),
      .fns = ~ !is.na(.x)
...
# Fit Forecast Models
Here we will fit ARIMA and GAM forecasts 
```{r fit_forecasts, message = FALSE, warning = FALSE,results = "hide"}
#==========
#ARIMA models
#==========
ARIMA100<-tsCV2(y=log(dat$Steelhead[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=NULL,
            h=FY,
            min=TY,
...
ARIMA100_withcov<-tsCV2(y=log(dat$Steelhead[-dim(dat)[1]]),
            Year=dat$Year,
...
ARIMA111_withcov<-tsCV2(y=log(dat$Steelhead[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select(all_of(vars))%>%as.matrix(),
            h=FY,
            min=TY,
...
ARIMA010_withcov<-tsCV2(y=log(dat$Steelhead[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select(all_of(vars))%>%as.matrix(),
            h=FY,
            min=TY,
...
ARIMA010<-tsCV2(y=log(dat$Steelhead[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=NULL,
            h=FY,
            min=TY,
...
# ARIMA111<-tsCV2(y=log(dat$Steelhead[-dim(dat)[1]]),
#             Year=dat$Year,
#             xreg=NULL,
#             h=FY,
#             min=TY,
...
#             order=c(1,1,1)
# )
ARIMA101_withcov<-tsCV2(y=log(dat$Steelhead[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select(all_of(vars))%>%as.matrix(),
            h=FY,
            min=TY,
...
ARIMA101<-tsCV2(y=log(dat$Steelhead[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=NULL,
            h=FY,
            min=TY,
...
#GAM models
#==========
gam<-tsCV2(y=log(dat$Steelhead[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=NULL,
            h=FY,
            min=TY,
...
# LFO10<-tsCV2(y=log(dat$Steelhead[-dim(dat)[1]]),
#             Year=dat$Year,
#             xreg=dat%>%dplyr::select(all_of(vars))%>%as.matrix(),
#             h=FY,
#             min=TY,
...
#   ,vars=vars
#   ,dat=dat
#   ,formula = as.formula(paste0("log(Steelhead) ~ gp(Year) + ",paste(vars,collapse = "+")))
...
#   ,vars=vars
#   ,dat=dat
#   ,formula = as.formula(paste0("log(Steelhead) ~ arma(p = 1, q = 1, cov=TRUE) + ",paste(vars,collapse = "+")))
#   ,prior = set_prior(horseshoe(df = 1, par_ratio = 0.5), class="b")
# )
```
...
#===================
forecasts<-list(
  ARIMA100 = ARIMA100$results
  ,ARIMA100_withcov = ARIMA100_withcov$results
  ,ARIMA111_withcov = ARIMA111_withcov$results
  #,ARIMA111 = ARIMA111$results
  ,ARIMA010_withcov = ARIMA010_withcov$results
  ,ARIMA010 = ARIMA010$results
  ,ARIMA101_withcov = ARIMA101_withcov$results
  ,ARIMA101 = ARIMA101$results
  ,gam = gam$results
  #,gp_horseshoe = LFO11$results
)%>%
...
ggplot(data=forecasts,aes(x=Year,y=Estimate,color=Model))+
  ...
  geom_line(size=1)+
  scale_color_brewer(palette="Spectral")+
  geom_point(data=dat,mapping=aes(x=Year,y=Steelhead),size=2,color="black")+
  ylim(0,NA)+
  theme_bw()
```
...
dat3<-forecasts%>%
  ...
  dplyr::select(Year,Model,Estimate,Steelhead)%>%
  mutate(error=Steelhead-Estimate)%>%
  filter(!is.na(error))%>%
  group_by(Model)%>%
  summarise(RMSE = sqrt(mean(error^2)),
            MAPE = mean(abs(error/Steelhead))*100,
            MSA = 100*(exp(mean(abs(log(Steelhead/Estimate))))-1)
              )%>%
  arrange(MSA)
...
for(i in 2: length(yrrange[1]:yrrange[2])){
  ...
  stackdat<-forecasts%>%
    ...
    left_join(dat%>%dplyr::select(Year,Steelhead))
  ...
  stacking_weights$Model<-colnames(stackdat)[!colnames(stackdat)%in%c("Year","Steelhead")]
  tdat<-tdat%>%
...
#=============================
dat4<-bind_rows(forecasts,ensembles)%>%
...
#==============
ggplot(data=ensembles,aes(x=Year,y=Estimate,color=Model))+
